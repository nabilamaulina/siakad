    ...
    <div class="card border-0 shadow-sm rounded-4 p-4 mb-4" style="background: #ffffff;">
        ...
        <div class="d-flex flex-wrap gap-1 align-items-center justify-content-start overflow-x-auto pb-1 style-scrollbar">
            <style>
                ...
                .btn-mhs-letter { font-size: 11px; width: 32px; height: 32px; padding: 0; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s; cursor: pointer; }
                .btn-mhs-letter.active { background-color: #245358 !important; color: #ffffff !important; font-weight: bold; }
            </style>
            ...
            <form id="form-tambah-mhs">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-secondary small fw-bold mb-1">Semester Berjalan *</label>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form id="form-edit-mhs" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4 row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label small fw-bold text-secondary">Semester Berjalan *</label>
                        <input type="number" name="semester_saat_ini" id="edit-semester" class="form-control rounded-3 bg-white text-dark small" required min="1" max="14">
                    </div>
                    <div class="col-md-6">
...
<script>
    ...
    function loadMahasiswa() {
        $.ajax({
            ...
            type: 'GET',
            data: { letter: currentLetter, search: currentSearch, angkatan: currentAngkatan, page: currentPage },
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if(data.status === 'success') {
                        Swal.fire({ icon: 'success', title: 'Berhasil!', text: data.message, confirmButtonColor: '#245358' });
                        $('#modalTambahMahasiswa').modal('hide');
                        $('#form-tambah-mhs')[0].reset();
                        loadMahasiswa();
                    } else {
                        Swal.fire({ icon: 'error', title: 'Gagal!', text: data.message, confirmButtonColor: '#245358' });
                    }
                }
            });
        });
        ...
        $(document).on('click', '#btn-buka-edit-modal', function() {
            ...
            setTimeout(function() { $('#modalEditMahasiswa').modal('show'); }, 400);
        });
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if(data.status === 'success') {
                        ...
                        Swal.fire({ icon: 'error', title: 'Error!', text: data.message, confirmButtonColor: '#245358' });
                    }
                }
            });
        });
        ...
        $(document).on('click', '.btn-hapus-mhs', function(e) {
            ...
            Swal.fire({
                title: 'Hapus data?', text: "Data '" + nama + "' akan dihapus permanen beserta akun loginnya!", icon: 'warning',
                showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#6c757d', confirmButtonText: 'Ya, Hapus!'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'proses.php', type: 'POST',
                        data: { action: 'delete', id_mahasiswa: id_mhs, csrf_token: '<?= $token; ?>' },
                        success: function(res) {
                            let data = (typeof res === 'object') ? res : JSON.parse(res);
                            if(data.status === 'success') { 
